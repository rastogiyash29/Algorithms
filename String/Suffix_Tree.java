...
public class Suffix_Tree {
	public static void main(String args[]) {
		...
		st.build();
		char[] patt = scn.nextLine().toCharArray();
		char[] text = s.toCharArray();
		ArrayList<Integer> list = new ArrayList<Integer>();
		st.dfs(patt, st.root, 0, list);
...
class SuffixTree {
	///////////////////////////////////////////////////////////////// Code starts
	///////////////////////////////////////////////////////////////// here
	public void dfs(char[] p, SuffixNode node, int i, ArrayList<Integer> list) {
		// Write Code here
		// APPLICATION 1: FIND all occurances of Pattern P in Text T
		if (input.length < p.length || node == null)
			return;

		int j = node.start;
		while (j <= node.end.end) {
			if (i == p.length - 1 && p[i] == input[j]) {
				if (node.index == -1)
		...
		dfs(p, node.child[p[i]], i, list);
	}

	static void findLeaves(SuffixNode node, ArrayList<Integer> list) {
		if (node == null)
			return;
		if (node.index == -1)
	...
	private void startPhase(int i) {
		...
		// global end for leaf. Does rule 1 extension as per trick 3 by incrementing
		// end.
		end.end++;
		...
		while (remainingSuffixCount > 0) {
			...
			if (active.activeLength == 0) {
				...
				// and break out of while loop. This is rule 3 extension and trick 2 (show
				// stopper)
				if (selectNode(i) != null) {
					active.activeEdge = selectNode(i).start;
					active.activeLength++;
					break;
				} // create a new leaf node with current character from leaf. This is rule 2
					// extension.
				else {
				...
				// if active length is not 0 means we are traversing somewhere in middle. So
				// check if next character is same as
				// current character.
				try {
					char ch = nextChar(i);
					// if next character is same as current character then do a walk down. This is
					// again a rule 3 extension and
					// trick 2 (show stopper).
					if (ch == input[i]) {
						// if lastCreatedInternalNode is not null means rule 2 extension happened before
						// this. Point suffix link of that node
						// to selected node using active point.
						// TODO - Could be wrong here. Do we only do this if when walk down goes past a
						// node or we do it every time.
						if (lastCreatedInternalNode != null) {
							lastCreatedInternalNode.suffixLink = selectNode();
						}
						// walk down and update active node if required as per rules of active node
						// update for rule 3 extension.
						walkDown(i);
						break;
					} else {
						// next character is not same as current character so create a new internal node
						// as per
						// rule 2 extension.
						...
						// if another internal node was created in last extension of this phase then
						// suffix link of that
						// node will be this node.
						if (lastCreatedInternalNode != null) {
							lastCreatedInternalNode.suffixLink = newInternalNode;
						}
						// set this guy as lastCreatedInternalNode and if new internalNode is created in
						// next extension of this phase
						// then point suffix of this node to that node. Meanwhile set suffix of this
						// node to root.
						...
						// if active node is root then increase active index by one and decrease active
						// length by 1
						else {
					...
					// this happens when we are looking for new character from end of current path
					// edge. Here we already have internal node so
					// we don't have to create new internal node. Just create a leaf node from here
					// and move to suffix new link.
					SuffixNode node = selectNode();
	...
	/**
	 * Do validation of the tree by comparing all suffixes and their index at leaf
	 * node.
	 */
